    ...
    private void transpositionCheckMenuItemActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_transpositionCheckMenuItemActionPerformed
        ...
        for (String word : words) {                                     //textArea'ya yazılan kelimeler tek tek for döngüsüne girip kontrol ediliyor .
            ...
            if (word.matches(".*\\d.*")) {                             //eğer bir sayı ile karşılaşırsa kontrol etmeden doğru kabul ediliyor .
                ...
                for (String str : tp.lists.get(0)) {                            // Transposition classında kelime uzunluguna göre oluşturulan arraylistten kelimeler
                    if (str.equals(word)) {                                     // sırayla kontrol ediliyor .
                ...
                if (condition) {                                                //eğer bu kelime word.txt de yoksa transposition var mı diye kontrol ediliyor .
                    ...
                    loop1:
                    for (String str : tp.lists.get(0)) {

                        for (int i = 0; i < str.length() - 1; i++) {

                            if (word.charAt(i) == str.charAt(i + 1) & word.charAt(i + 1) == str.charAt(i)) { // transposition'u ardışık harflerde aradık . 
                ...
                if (condition) {
                    for (String str : tp.lists.get(1)) {
                        ...
                        for (int i = 0; i < str.length() - 1; i++) {
                            ...
                            if (word.charAt(i) == str.charAt(i + 1) & word.charAt(i + 1) == str.charAt(i)) {
                                ...
                                String tWord = String.valueOf(wordTemp);

                                if (tWord.equals(str)) {

                                    undo.addTextMethod();
                                    textArea.setText(textArea.getText().replace(word, tWord));/////////////////////////////////////////////////
                        ...
                        for (int i = 0; i < str.length() - 1; i++) {
                            ...
                            if (word.charAt(i) == str.charAt(i + 1) & word.charAt(i + 1) == str.charAt(i)) {
                                ...
                                if (tWord.equals(str)) {
                                    ...
                                    undo.addTextMethod();
                                    textArea.setText(textArea.getText().replaceFirst("(?i)" + word, tWord));
                ...
                if (condition) {
                    loop1:
                    for (String str : tp.lists.get(8)) {
                        if (wordLength != str.length()) {                       // ondan büyük uzunluklu kelimeler tek listede toplandığı için öncelikle
                            // kelime uzunluğunu kontrol ediyoruz . Kelime uzunlukları aynı ise üstte yaptıgımız 
                            break;                                              //kontroller yapılıyor .
                        }

                        for (int i = 0; i < str.length() - 1; i++) {
                            System.out.println("str:" + str + "length:" + (str.length() - 1));
                            if (word.charAt(i) == str.charAt(i + 1) & word.charAt(i + 1) == str.charAt(i)) {
        ...
        for (int i = 0; i < dgHighlights.size(); i = i + 2) {
            try {
                h2.addHighlight(dgHighlights.get(i), dgHighlights.get(i + 1) - 1, darkGreen); // koyu yeşil highlight işlemi (singletransposition kelimeler )

            } catch (BadLocationException ex) {
                Logger.getLogger(TextEditorJFrame.class.getName()).log(Level.SEVERE, null, ex);
            }
        }
        for (int i = 0; i < rHighlights.size(); i = i + 2) {
            try {
                h2.addHighlight(rHighlights.get(i), rHighlights.get(i + 1) - 1, red);       // yanlış kelimeler için kırmızı highlight
            } catch (BadLocationException ex) {
                Logger.getLogger(TextEditorJFrame.class.getName()).log(Level.SEVERE, null, ex);
            }
        }
        for (int i = 0; i < gHighlights.size(); i = i + 2) {
            try {
                h2.addHighlight(gHighlights.get(i), gHighlights.get(i + 1) - 1, green);  // doğru kelimeler için yeşil highlight
            } catch (BadLocationException ex) {
                Logger.getLogger(TextEditorJFrame.class.getName()).log(Level.SEVERE, null, ex);
            }
